...
const CLI_CONFIGS = {
  ...
  'claude-kimi': {
    ...
    env: { ANTHROPIC_API_KEY: process.env.SILICONFLOW_KIMI_API_KEY },
    model: 'Kimi-K2.5'
  },
  ...
  'claude-glm': {
    command: 'claude',
    args: ['--print', '--permission-mode', 'bypassPermissions'],
    env: { ANTHROPIC_API_KEY: process.env.SILICONFLOW_GLM_API_KEY },
    model: 'GLM-5'
  },
  // Codex CLI - Kimi (并行任务)
  'codex-kimi': {
    command: 'codex',
    args: ['--full-auto'],
    env: { OPENAI_API_KEY: process.env.SILICONFLOW_KIMI_API_KEY },
    model: 'Kimi-K2.5'
  },
  // Codex CLI - GLM (工程任务)
  'codex-glm': {
    command: 'codex',
    args: ['--full-auto'],
    env: { OPENAI_API_KEY: process.env.SILICONFLOW_GLM_API_KEY },
    model: 'GLM-5'
  },
  // 默认 Claude
  claude: {
